...
species_abundance_matrix <- filtered_df %>%
...
transposed_data <- t(species_abundance_matrix)

accumulation_curves <- specaccum(transposed_data)
...
# minimum is 89 # so rarefy by this sample size aka rarefaction depth  
...
rarefied_richness_per_depth_zone<-as.data.frame(rarefied_richness_per_depth_zone)

plot(rarefied_richness_per_depth_zone, col = "purple", cex = 1.8, cex.lab=1, pch = 20, ylab = "Depth Zone", 
     xlab = "Richness")
...
ggplot(df_richness_long, aes(x = factor(DepthZone), y = Richness, fill = DepthZone)) +
  geom_boxplot() +
  xlab("Depth Zone") +
  ...
  ggtitle("") +
  theme(
    axis.text = element_text(size = 15),  # Change the size of axis labels
    axis.title = element_text(size =20),  # Change the size of axis titles
    plot.title = element_text(size = 20)  # Change the size of the plot title
  )
...
rank_abundance_plot<-radfit(rarefied_matrix)
plot(rank_abundance_plot)

# finding out which species are most abundant in each depth zone 

# Extract the species that are ranked first in each depth zone.
most_abundant_species <- apply(species_abundance_matrix, 1, function(depth_zone) {
  # Check for missing values and exclude them when finding the maximum.
  max_species_index <- which.max(depth_zone)
  if (!is.na(max_species_index)) {
    colnames(abundance_matrix)[max_species_index]
  } else {
    "No Data"  # You can replace "No Data" with an appropriate value for missing data.
  }
})

result <- data.frame(DepthZone = rownames(abundance_matrix), MostAbundantSpecies = most_abundant_species)
...
# Find the column heading (species name) with the maximum value in row 3.
most_abundant_species <- colnames(rarefied_matrix)[which.max(rarefied_matrix[row_to_analyze, ])]
