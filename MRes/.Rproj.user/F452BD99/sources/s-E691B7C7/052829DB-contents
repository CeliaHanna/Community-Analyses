...
# just plotting coral rubble 

subset_coralrubble <- substrate_counts[substrate_counts$substrate == "coral rubble", substrate_counts$count == "count" ]

coral_rubble_data <- substrate_counts[substrate_counts$substrate == "Coral rubble", ]


all_coral_rubble<- data.frame(
  Depth_Zone = c("0-200m", "200-400m", "400-600m", "600-800m", "1000-1200m", "1200-1300m"),
  Substrate = "Coral rubble",
  Count = c(284, 186, 253, 0, 0, 0)  # Set count to 0 for depth zones with no coral rubble
)

# explicitly set factor levels so depth zones are in correct order 

all_coral_rubble$Depth_Zone <- factor(
  all_coral_rubble$Depth_Zone,
  levels = c("0-200m", "200-400m", "400-600m", "600-800m", "1000-1200m", "1200-1300m")
)


ggplot(all_coral_rubble, aes(x = `Depth_Zone`, y = Count)) +
  geom_bar(stat = "identity", col = "lightgrey") +
  labs(x = "Depth Zone", y = "Count") +
  ggtitle("Coral Rubble Count by Depth Zone") +
  theme_minimal()

ggplot(all_coral_rubble, aes(x = `Depth_Zone`, y = Count, fill = `Depth_Zone`)) +
  geom_bar(stat = "identity") +
  labs(x = "Depth Zone", y = "Count") +
  ggtitle("") +
  theme_minimal() +
  scale_fill_manual(values = c("darkgrey", "darkgrey", "darkgrey", "orange", "orange", "orange"))  # Set custom bar colors
